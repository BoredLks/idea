%% QoE优化算法 - 小时间尺度视频质量适应 (并行版本)
% 基于第一个算法的缓存决策，优化单个时隙单个社区的QoE
clc;
close all;
clear;
...
%% 主计算循环
qoe_algorithm_number = zeros(7, 10); %7个算法，计算number从1到10。
hit_rate_algorithm_number = zeros(7, 10); %7个算法，计算number从1到10。
wait_time_algorithm_number = zeros(7, 10); %7个算法，计算number从1到10。
for number = 1:10
    fprintf('处理参数 %d/10...\n', number);
    
    % p_sbs = 50*number;     % SBS计算资源，单位: Mbit/s
    % p_iu = 10*number;      % IU计算资源，单位: Mbit/s

    D_eg = 5*number * nf * v_chi_star; % SBS缓存容量
    D_iu = 2*number * nf * v_chi_star; % IU缓存容量

    % gamma_m = 0.1*number;             % Zipf分布参数：越接近1，最热门的视频被请求概率越大

    % Per_m = 0.02*number;               % 每个社区选作缓存节点的用户比例
    % iu_count_per_community = round(user_per_community * Per_m);

    % K = 0.2*number;                     % 系统参数

    % user_per_community = 10*number;   % 每个社区的用户数量
    % total_users = h * user_per_community;
    % iu_count_per_community = round(user_per_community * Per_m);
    for algorithm_num = 1:7
        ...
        parfor run_now = 1:run_num
            % 设置随机数种子确保可重现性
            %rng(run_now + algorithm_num * 1000 + zipf * 10000);
        ...
        sbs_positions, per_user_positions, now_user_positions, user_community);
                ...
                hit_rate(run_now) = cache_hit_rate;
                wait_time(run_now) = initial_wait_time;
        ...
        hit_rate_algorithm_number(algorithm_num, number) = mean(hit_rate);
        wait_time_algorithm_number(algorithm_num, number) = mean(wait_time);
        fprintf('    算法 %d 完成，平均QoE: %.6f\n', algorithm_num, qoe_algorithm_number(algorithm_num, number));
    end
end
...
x_data = (1:number)*10;
plot(x_data, qoe_algorithm_number(1,:), '-o', 'LineWidth', 3);
hold on;
...
xlim([1*10, number*10]);
xticks(x_data);
xlabel('Cache size of System (Files)','FontSize', 18);
ylabel('QoE','FontSize', 18);
lgd = legend('Proposed', 'Random-IU', 'LMPC', 'RAC', 'HC', 'GMPC', 'NCC', 'Location', 'best');
lgd.FontSize = 12;
grid on;

saveas(gcf,'Figure\main.fig');

figure('Position', [100, 100, 800, 600]);
plot(x_data, hit_rate_algorithm_number(1,:), '-o', 'LineWidth', 3);
hold on;
plot(x_data, hit_rate_algorithm_number(2,:), '-*', 'LineWidth', 3);
plot(x_data, hit_rate_algorithm_number(3,:), '-x', 'LineWidth', 3);
plot(x_data, hit_rate_algorithm_number(4,:), '-square', 'LineWidth', 3);
plot(x_data, hit_rate_algorithm_number(5,:), '-diamond', 'LineWidth', 3);
plot(x_data, hit_rate_algorithm_number(6,:), '-v', 'LineWidth', 3);
plot(x_data, hit_rate_algorithm_number(7,:), '-^', 'LineWidth', 3);
xlim([1*10, number*10]);
xticks(x_data);
xlabel('Cache size of System (Files)','FontSize', 18);
...
saveas(gcf,'Figure\Hit_rate.fig');

figure('Position', [100, 100, 800, 600]);
plot(x_data, wait_time_algorithm_number(1,:), '-o', 'LineWidth', 3);
hold on;
plot(x_data, wait_time_algorithm_number(2,:), '-*', 'LineWidth', 3);
plot(x_data, wait_time_algorithm_number(3,:), '-x', 'LineWidth', 3);
plot(x_data, wait_time_algorithm_number(4,:), '-square', 'LineWidth', 3);
plot(x_data, wait_time_algorithm_number(5,:), '-diamond', 'LineWidth', 3);
plot(x_data, wait_time_algorithm_number(6,:), '-v', 'LineWidth', 3);
plot(x_data, wait_time_algorithm_number(7,:), '-^', 'LineWidth', 3);
xlim([1*10, number*10]);
xticks(x_data);
xlabel('Cache size of System (Files)','FontSize', 18);
ylabel('Waiting time','FontSize', 18);
lgd = legend('Proposed', 'Random-IU', 'LMPC', 'RAC', 'HC', 'GMPC', 'NCC', 'Location', 'best');
lgd.FontSize = 12;
grid on;

saveas(gcf,'Figure\Waiting_time.fig');
